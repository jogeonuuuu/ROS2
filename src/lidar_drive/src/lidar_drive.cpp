...
void Final::error_calc(cv::Mat &lidar_img) {
    ...
    cv::Point left_min_point(left_area.cols/2 - 1 + 60, left_area.rows-1); //좌표로 접근해야되기 때문에 +50 (대략 1.5m 내에 장애물 인식)
    ...
    cv::Point right_min_point((right_area.cols/2 - 1) - 30, right_area.rows-1);
    ...
    for(int i = 1; i < lable_cnt; i++) {
        ...
        // cv::rectangle(lidar_img, cv::Rect(q[0],q[1],q[2],q[3]), cv::Scalar(0,255,255), 2);
        // cv::rectangle(lidar_img, cv::Rect(p[0],p[1],p[2],p[3]), cv::Scalar(0,255,255), 2);
    }
    ...
    for(int i = 1; i < lable_cnt; i++) {
        ...
        // cv::rectangle(lidar_img, cv::Rect(p[0]+(lidar_img.cols/2),p[1],p[2],p[3]), cv::Scalar(0,255,255), 2);
    }
